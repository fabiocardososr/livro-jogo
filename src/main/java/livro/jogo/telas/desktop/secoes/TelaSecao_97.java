...
public class TelaSecao_97 extends TelaSecoesBasica {
    public TelaSecao_97(Secao secao) {
        super(secao);
    }
        ...
        botaoOpcao1.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getSource() == botaoOpcao1)
                    if (UtilBolsa.verificarExistenciaDeItemNaBolsa(ItensMapeamento.PROVISAO.getIdItem()))
                        abrirProximaSecao( secao.getProximasSecoes().getFirst().getCodProximaSecao() );
                    else
                        CarregarTelas.telaMensagem(DadosLivroCarregado.getPersonagem().getNome()+
                                ",\n\nVocê não tem nenhuma provisão para comer!");
            }
