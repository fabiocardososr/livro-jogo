...
import livro.jogo.acaosecoes.AcoesSecao_196;
...
public class TelaSecao_196 extends TelaSecoesBasica {
    boolean possuiRedeDeAprisionamento;

    public TelaSecao_196(Secao secao) {
        super(secao);

        new Util().reproduzirAudioMp3("audio/efeitos_sonoros/passos.mp3", null);

        addWindowListener(new WindowListener() {
            @Override
            public void windowOpened(WindowEvent e) {
                possuiRedeDeAprisionamento = AcoesSecao_196.verificaSePossuiRedeDeAprisionamento();
            }
    ...
    protected void acaoBotoes(Secao secao) {
        ...
        botaoOpcao2.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if ( possuiRedeDeAprisionamento )
                   abrirProximaSecao( secao.getProximasSecoes().get(1).getCodProximaSecao() );
                else
                    CarregarTelas.telaMensagem(DadosLivroCarregado.getPersonagem().getNome()+
                            ",\n\nvocê não possui uma Rede de Aprisionamento.");
