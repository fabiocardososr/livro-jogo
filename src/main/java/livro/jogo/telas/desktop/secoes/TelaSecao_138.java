...
import livro.jogo.acaosecoes.AcoesSecao_138;
...
public class TelaSecao_138 extends TelaSecoesBasica {
    private boolean possuiPocaoControleDePlantas;
    private TelaSecoesBasica tela = this;

    public TelaSecao_138(Secao secao) {
        super(secao);

        addWindowListener(new WindowListener() {
            @Override
            public void windowOpened(WindowEvent e) {
                possuiPocaoControleDePlantas = AcoesSecao_138.verificaSePossuiPocaoControleDePlantas();
            }
    ...
    protected void acaoBotoes(Secao secao) {
        botaoOpcao1.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if ( possuiPocaoControleDePlantas )
                    abrirProximaSecao( secao.getProximasSecoes().getFirst().getCodProximaSecao() );
                else
                    CarregarTelas.telaMensagem(DadosLivroCarregado.getPersonagem().getNome() +
                            ",\n\nvocê não possui uma Poção de Controle de Plantas." +
        ...
        botaoOpcao2.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (possuiPocaoControleDePlantas) {
                    CarregarTelas.telaMensagem(DadosLivroCarregado.getPersonagem().getNome() +
                            ",\n\nvocê possui uma Poção de Controle de Plantas." +
                            "\nPrefere lidar com as ERVAS ENREDANTES sem ajuda da poção?", tela);
                }

                if ( (!possuiPocaoControleDePlantas) || (isRespostaTelaMensagem()) )
